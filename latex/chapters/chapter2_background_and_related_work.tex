...
在順向運動學中，Denavit-Hartenberg 方法是描述機器人運動學的最常用方法。這些參數 $a_{i-1}$、1i−α、di 和 θi 分別是力臂長度(Link length)、節點扭曲(Link twist)、節點偏移(Link offset)和節點角度(joint angle)。每個關節都附有一個座標系來決定這些參數。

如圖2所示，沿Xi-1測得的Zi-1到Zi的距離記ai-1，沿Xi測得的Zi-1和Zi之間的角度為αi-1，距Xi的距離沿Zi測量的-1 到Xi 指定為di，關於Zi 測量的Xi-1 到Xi 之間的角度指定為θi (Craig, 1989)。單一鏈路的一般變換矩陣T1ii−可以如下獲得。
