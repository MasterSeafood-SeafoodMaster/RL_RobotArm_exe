...
在順向運動學中，Denavit-Hartenberg 方法是描述機器人運動學的最常用方法。這些參數 ai-1、1i−α、di 和 θi 分別是力臂長度(Link length)、節點扭曲(Link twist)、節點偏移(Link offset)和節點角度(joint angle)。每個關節都附有一個座標系來決定這些參數。
